...
public class BaoBiaoController {
	...
	public String list1(Model model, SalarySelectForm form) {
		...
		model.addAttribute("listEmployees", listEmployees);
		model.addAttribute("deptList", departmentsList);
//		model.addAttribute("salarySelectForm", SalarySelectForm);
		...
		for (SalarySelectForm salarySelectForm : listSalarySelectForm) {
			totalSalary += salarySelectForm.getMoney()[0]; // 假设 money 数组中只有一个元素
		}
...
//		Long employeeRank;
		for (SalarySelectForm salarySelectForm : listSalarySelectForm) {
			Long employeeId = salarySelectForm.getEmployeeId();
			Long employeeRank = employeesService.selectRankByEmployeeId(employeeId);
			salarySelectForm.setEmployeeRank(employeeRank);
			model.addAttribute("employeeRank", employeeRank);
			break; // 仅获取第一个元组的薪资等级信息
		}
	...
	public String listDepartments1(SalarySettlementsForm form, Model model) {
		...
		for (DateDepartmentsForm dateDepartments : listDateDepartmentsForm) {
			System.out.println(dateDepartments);//检验
	...
	public String list(Long employeeId, Model model) {
		...
		// 新增代码，获取员工薪资等级信息
		Long empRank = employeesService.selectRankByEmployeeId(employeeId);
