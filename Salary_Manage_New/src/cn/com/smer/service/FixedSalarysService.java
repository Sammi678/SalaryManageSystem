...
public class FixedSalarysService {
	...
	public boolean updateEmployeeRank(Map<String, Long> paramMap) {
		...
		boolean result = employeesService.updateEmployeeRank(paramMap);
		return result;
	}
	...
	//通过员工编号修改固定项目
	// 添加通过员工编号修改固定项目的函数
	public boolean updateByEmployeeId(Long employeeId, FixedSalarysForm fsForm) {
		// 创建 FixedSalarys 对象并设置员工编号
		FixedSalarys fs = new FixedSalarys();
		fs.setEmployeeId(employeeId);

		// 使用 BeanUtils 复制属性值
		BeanUtils.copyProperties(fsForm, fs);

		// 调用固定项目 DAO 层方法执行更新操作
		int count = fixedSalarysDao.updateByEmployeeId(employeeId);
